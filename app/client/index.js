...
export default function Client({
  ...
  const uniqueLabels = useMemo(() => {
    ...
    return [...new Set(initialLabels.map((l) => l.title))].sort((a, b) =>
  ...
  const handleLabelToggle = useCallback(
    (labelTitle) => {
      ...
      newSelected.has(labelTitle)
        ? newSelected.delete(labelTitle)
        : newSelected.add(labelTitle);
  ...
  const accountDisplayName = useMemo(
    () => user?.zalo?.name || "Chưa chọn tài khoản",
    [user],
  );
  const scheduleData = useMemo(
    () => Array.from(selectedCustomerMap.values()),
    [selectedCustomerMap],
  );
