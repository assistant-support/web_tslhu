...
const useTraCuuData = (phones) => {
    ...
    useEffect(() => {
        ...
        const fetchOne = async (phone) => {
            try {
                const res = await fetch('https://tapi.lhu.edu.vn/TS/AUTH/XetTuyen_TraCuu', {
                    method: 'POST',
...
const Row = React.memo(function Row({ row, rowIndex, onToggle, checked, onRowClick, onSearch }) {
    const hasData = row._apiStatus === 'success';
    const canSearch = hasData && row.MaDangKy;
    const disabledStyle = { backgroundColor: 'var(--red)', cursor: 'default', pointerEvents: 'none' };
    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{ flex: 1, padding: '0 16px', overflow: 'visible', display: 'flex', gap: 8, alignItems: 'center' }}
            >
                <div
                    style={{
                        flex: 1,
                ...
                >
                    ...
                    <WrapIcon
                        ...
                        content="Tra cứu"
                        placement="left"
                        style={hasData ? { backgroundColor: 'var(--main_d)', cursor: 'pointer' } : disabledStyle}
                        click={(e) => {
                            e.stopPropagation();
...
export default function Client({ initialData, initialPagination, initialLabels, user }) {
    ...
    const { selectedIds, toggleOne: toggleSelectRow, setSelectedIds, size: selectedCount } = useSelection();
    ...
    useEffect(() => {
        const t = setTimeout(() => {
            if (query !== (searchParams.get('query') || '')) handleNavigation('query', query);
        }, 300);
        return () => clearTimeout(t);
    }, [query, searchParams, handleNavigation]);

    const handleRefresh = useCallback(() => startTransition(() => router.refresh()), [router]);

    /* ------ LABEL / FILTER ------ */
    const uniqueLabels = useMemo(
        () => (initialLabels ? [...new Set(initialLabels.map((l) => l.title))].sort((a, b) => a.localeCompare(b, 'vi')) : []),
        [initialLabels]
    );
    ...
    const allOnPageChecked = useMemo(
        () => initialData.length > 0 && initialData.every((r) => selectedIds.has(r._id)),
    ...
    const handleTogglePageAndStoreData = useCallback(() => {
        ...
        setSelectedIds((prev) => {
            ...
            pageIds.forEach((id) => (allOnPageChecked ? next.delete(id) : next.add(id)));
            return next;
        });
    }, [initialData, allOnPageChecked, setSelectedIds]);

    /* ------ ROWS HIỂN THỊ & API TRA CỨU ------ */
    const scheduleData = useMemo(() => [...selectedCustomerMap.values()], [selectedCustomerMap]);
    const rowsToDisplay = useMemo(() => (viewMode === 'selected' ? scheduleData : initialData), [viewMode, scheduleData, initialData]);
    const visiblePhones = useMemo(() => rowsToDisplay.slice(0, 10).map((r) => r.phone), [rowsToDisplay]);
    ...
    const totalDisplayPages = useMemo(
        () => (viewMode === 'selected' ? Math.ceil(scheduleData.length / serverLimit) || 1 : serverTotalPages),
            ...
            <div className={styles.filterSection}>
                <div className={styles.filterHeader}>
                    ...
                    <div style={{ display: 'flex', gap: 16 }}>
                        <button className={styles.btnAction} onClick={() => setHistoryOpen(true)}>
                            Xem lịch sử
                        </button>
                ...
                <div className={styles.filterControls}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        <div className={styles.viewToggle}>
                            <button className={`${styles.viewBtn} ${viewMode === 'all' ? styles.active : ''}`} onClick={() => setViewMode('all')}>
                                Tất cả ({initialPagination?.total || 0})
                            </button>
                            <button className={`${styles.viewBtn} ${viewMode === 'selected' ? styles.active : ''}`} onClick={() => setViewMode('selected')}>
            ...
            <div className={styles.messageSection}>
                <div className={styles.accountSelector} style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex' }}>
                        <div
                            className="input"
                            style={{ width: 150, padding: '8px 12px', color: '#495057', borderRight: 'none', borderRadius: '5px 0 0 5px', display: 'flex', alignItems: 'center' }}
                        >
                        ...
                        <div className={styles.gridHeader}>
                            <div style={{ display: 'flex', flex: 5 }}>
                                <div className={`${styles.gridCell} ${styles.colTiny}`} style={{ textAlign: 'center', flex: 0.5 }}>
                                    ...
                                    />
                                    {selectedCount > 0 && (
                                        <span className={styles.selectedCount} style={{ color: 'white' }}>
                                            {selectedCount}
                                        </span>
                                    )}
                                </div>
                                <div className={`${styles.gridCell} ${styles.colSmall} text_6`} style={{ flex: 0.5, color: 'white' }}>
                                    STT
                                </div>
                            ...
                            <div className={`${styles.gridCell} text_6`} style={{ color: 'white', flex: 1 }}>
                                Cập nhập
                            </div>
                        </div>
                        ...
                        <div className={styles.gridBody}>
                            {rowsToDisplay.map((r, idx) => (
                                <Row
                                    ...
                                    rowIndex={(viewMode === 'all' ? (serverPage - 1) * serverLimit : 0) + idx}
                    ...
                    {totalDisplayPages > 1 && (
                        <div className={styles.pagination}>
                            {currentDisplayPage > 1 && (
                                <button onClick={() => handleNavigation('page', currentDisplayPage - 1)} className={styles.pageBtn}>
                                    &laquo; Trang trước
                                </button>
                            )}
            ...
            {showLabelPopup && (
                <div className={styles.labelModalBackdrop} onClick={() => setShowLabelPopup(false)}>
                    <div className={styles.labelModal} onClick={(e) => e.stopPropagation()}>
                        ...
                        <button className={styles.btnCloseModal} onClick={() => setShowLabelPopup(false)}>
                            Đóng
                        </button>
                    </div>
                </div>
                    ...
                    <table className={styles.popupTable}>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style={{ textAlign: 'right' }}>{traCuuData.TongDiem?.toFixed?.(2) ?? traCuuData.TongDiem}</td>
